...
class ToolsController @Inject()(cc: ControllerComponents,
  ...
  def blastRun = Action(parse.multipartFormData) { implicit request =>
    ...
    var db = data.blastType match {
      case "blastn" =>Utils.path + "/blastData/cds/" + data.db
      case "blastnGenome" => Utils.path + "/blastData/genome/" + data.db
      case "blastp" => Utils.path + "/blastData/pep/" + data.db
    }
    ...
    val command1 = Utils.path + "/tools/ncbi-blast-2.6.0+/bin/%s%s -query ".format(blastType, Utils.suffix) + seqFile.getAbsolutePath + " -subject " +
      db + " -outfmt 5 -evalue " + data.evalue + " -max_target_seqs " + data.maxTargetSeqs +
  ...
  def blastResultBefore(path: String, types: String): Action[AnyContent] = Action {
    implicit request =>
      Ok(views.html.english.tools.blastResult(path,types))
  }
  ...
  def seqRegion = Action { implicit request =>
    ...
    val command = if (new File(windowsPath).exists()) {
      Utils.path + "/tools/samtools-0.1.19/samtools.exe faidx " + Utils.path + "/blastData/genome.fasta " + data.region
    } else {
      "samtools faidx " + Utils.path + "/blastData/genome.fasta " + data.region
    }
