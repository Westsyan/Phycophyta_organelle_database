...
class UtilsController @Inject()(cc: ControllerComponents)(implicit exec: ExecutionContext) extends AbstractController(cc) {
  ...
  def getCpimg(id: Int) = Action { implicit request =>
    val path = s"${Utils.path}/images/cp_img/cp${id}.jpg"
  ...
  def getCpOgdraw(gb:String) = Action{implicit request=>
    val path = s"${Utils.path}/images/cp_ogdraw/${gb}.jpg"
    val file = new File(path)
    SendImg(file,request.headers)
  }
  ...
  def getLastzTarfetExample = Action { implicit request =>
    val line = Utils.readFileToString(Utils.path + "/download/1.fasta")
    Ok(Json.toJson(line))
  }

  def downloadLastzTarfetExample = Action { implicit request =>
    Ok.sendFile(new File(Utils.path + "/download/1.fasta")).withHeaders(
      //缓存
  ...
  def getLastzQueryExample = Action { implicit request =>
    val line = Utils.readFileToString(Utils.path + "/download/2.fasta")
    Ok(Json.toJson(line))
  }

  def downloadLastzQueryExample = Action { implicit request =>
    Ok.sendFile(new File(Utils.path + "/download/2.fasta")).withHeaders(
      //缓存
      CACHE_CONTROL -> "max-age=3600",
